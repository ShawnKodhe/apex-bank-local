...
export default function Deposit() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!walletId || !amount || parseFloat(amount) <= 0) return;
    setLoading(true);
    ...
    await base44.entities.Transaction.create({
      ...
      status: "pending",
      description: description || `Deposit to ${wallet?.wallet_name}`,
    ...
    setLoading(false);
    setSuccess(true);
    toast({ title: "Deposit submitted", description: "Your deposit is being processed." });
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-heading font-bold tracking-tight">Deposit Funds</h1>
        <p className="text-muted-foreground mt-1">Add money to your wallet</p>
      </div>
      ...
      {success ? (
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-8 text-center animate-fade-in">
          ...
          <h2 className="text-xl font-heading font-bold text-emerald-700">Deposit Submitted!</h2>
          <p className="text-sm text-emerald-600 mt-2">Your deposit of ${parseFloat(amount || 0).toFixed(2)} is being processed</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="bg-card rounded-2xl border border-border p-6 space-y-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 className="font-heading font-semibold">New Deposit</h2>
              <p className="text-xs text-muted-foreground">Select wallet and enter amount</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {wallets.length === 0 && (
              <p className="text-xs text-muted-foreground">No active wallets. Add a wallet first.</p>
            )}
          </div>
          ...
          <Button
            type="submit"
            disabled={loading || !walletId || !amount || parseFloat(amount) <= 0}
            className="w-full h-12 rounded-xl text-base"
          >
            {loading ? "Processing..." : `Deposit $${parseFloat(amount || 0).toFixed(2)}`}
          </Button>
        </form>
      )}
    </div>
